//! The content of this file is derived from https://github.com/rp-rs/i2c-pio-rs licensed under the Apache License 2.0.
//!
//! Apache License
//! Version 2.0, January 2004
//! http://www.apache.org/licenses/
//!
//! Modifications to the original file are:
    ...
    Function<P>: ValidPinMode<SDA> + ValidPinMode<SCL>,
{
        ...
        Function<P>: ValidPinMode<SDA> + ValidPinMode<SCL>,
    {
        ...
        let sda = sda.into_floating_input();
        let scl = scl.into_floating_input();
    ...
    Function<P>: ValidPinMode<SDA> + ValidPinMode<SCL>,
{
    ...
    fn exec<'a>(
        ...
        let mut first = true;
        let mut following_read: bool = false; // if the operation is following read operation
        for op in operations {
            match op {
                Operation::Read(buf) => {
                    self.setup(address, true, !first);
                ...
                Operation::Write(buf) => {
                    if first || following_read {
                        self.setup(address, false, !first);
                    }
                    self.write(buf.iter().cloned());
                    following_read = false;
                }
            };
            first = false;
            if self.has_errored() {
